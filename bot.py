...
def build_morning_header(user_id):
    ...
    if work:
        try:
            ...
            lines.append(f"🚌 Выходи в {leave_h:02d}:{leave_m:02d} (дорога ~{travel} мин, к {arrival} на работе)")
        except:
            pass
    return "\n".join(lines)
...
def get_weather(city):
    try:
        ...
        weather = d["weather"][0]
        max_t = weather["maxtempC"]
        min_t = weather["mintempC"]
        emoji = "☀️"
        desc_l = desc.lower()
        if "дождь" in desc_l or "ливень" in desc_l: emoji = "🌧"
        elif "облач" in desc_l or "пасмурн" in desc_l: emoji = "☁️"
        elif "снег" in desc_l: emoji = "❄️"
        elif "туман" in desc_l: emoji = "🌫"
        elif "гроза" in desc_l: emoji = "⛈"
        elif "переменн" in desc_l: emoji = "⛅️"
        return (f"{emoji} {desc}\n"
                f"🌡 {temp}°C (ощущается {feels}°C)\n"
                f"📊 Днём {max_t}° / ночью {min_t}°\n"
                f"💨 Ветер {wind} км/ч\n"
